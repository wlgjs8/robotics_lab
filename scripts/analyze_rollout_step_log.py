    ...
    *,
    ...
    for arm in ARMS:
        ...
        print(
            ...
            f"meas_pose={valid_meas} descent_segments={len(segments)}"
        )
        for number, segment in enumerate(segments, start=1):
            ...
            blocked_like = (
                follow_ratio is not None and follow_ratio < blocked_follow_ratio
            )
            print(
        ...
        if not segments:
            ...
            diagnoses.append(
                f"{arm}: cmd 하강 대비 meas 추종이 작음 → 서보/기구 차단 후보"
            )
        else:
            diagnoses.append(
    ...
    print()
    print("판정 힌트:")
    print("  cmd z가 내려가는데 meas z가 안 따라감 → 서보/기구 차단 가능성")
...
def save_plot(
    ...
    figure, axes = plt.subplots(3, 2, figsize=(14, 9), sharex="col")
    for column, arm in enumerate(ARMS):
        ...
        axes[2][column].set_xlabel("t - first sample (s)")
