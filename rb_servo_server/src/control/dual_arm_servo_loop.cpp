...
// asked the left wrist for 53-84 deg/s, the follower core capped it at 0.9 rad/s and
// the output SMD + IK throttle lagged a further 5.5 deg (and, found afterwards, 557 of
// the window's segment solves were Ruckig-refused because the target-velocity clamp
// sat 5e-10 above v_max -- chunk_follower_core.hpp clampInside), so the PLAN sat 6 deg
// ahead of the SENT command while the arm tracked that command to 0.7-1.1 deg. That
// is the lag of this chain, not the robot leaving the plan. A soft breach while the robot
// tracks its command (dead-time tolerant, see CommandTrackingWindow) is now excused;
// only these hard bounds still latch regardless, as the runaway backstop.
constexpr double kPoseTrackDivergenceHardPosTolM = 0.10;
constexpr double kPoseTrackDivergenceHardAngTolRad = 0.30;   // 17.2 deg
// The recent-command window length lives with the windows (DualArmServoLoop::kCommandTrackingWindowTicks).
...
ArmCommand applyPoseTrackSmd(
    ...
    // THE contact normal, stand frame (ForceGate::contactNormal: the unit measured
    // physical force = the free-space direction). Null or zero = no contact.
    const math::Vector3* contact_normal_stand = nullptr,
    // Out: this tick's DEMAND [m/s] - the translation step the raw command integrated
    // into the goal, over dt. The gate never touches the goal (it holds the tracker's
    // STATE) and the fold is declined on this path, so this can never become a
    // function of the gate's own output or of the law's yield. 0 when the call only
    // (re)latched the command reference. The goal-vs-command distance it replaces
    // carried the engagement offset (a 5 mm standing offset read as 2.5 m/s).
    double* source_demand_m_s = nullptr
) {
    ...
    if (overlay != nullptr) reference = overlay->strip(reference);
    if (!tracker->active()) {
        if (seed_stand_twist != nullptr) {
            tracker->reset(reference, *seed_stand_twist);
        } else {
    ...
    const double goal_step_m = tracker->updateGoalFromCommand(command.tcp_target_stand);
    if (source_demand_m_s != nullptr) {
        *source_demand_m_s = dt_sec > 0.0 ? goal_step_m / dt_sec : 0.0;
    }
    // THE CONTACT NORMAL is the servo loop's one per-tick F_hat (2026-09-15); zero
    // means no contact above the noise band and the gate is not applied at all.
...
bool ruckigFollowerConfigChanged(const RuckigFollowerConfig& a, const RuckigFollowerConfig& b) {
    return a.enable != b.enable ||
        ...
        a.output_smd.velocity_ff_linear_gain != b.output_smd.velocity_ff_linear_gain ||
        a.output_smd.profile_feedforward != b.output_smd.profile_feedforward ||
...
DualArmServoLoop::DualArmServoLoop(
    ...
    if (config.force_control.enable) {
        ...
        // opened is a law nobody can check against what the arm actually did. This is
        // the banner an operator predicts the robot from, so it prints the derived b,
    ...
    if (config_.safety.self_collision.enable) {
        ...
        collision_monitor_cfg_.environment_d_hard_m =
            inherit(m.environment.d_hard_m, m.d_hard_m);
        collision_monitor_cfg_.environment_d_slow_m =
            inherit(m.environment.d_slow_m, m.d_slow_m);
        collision_monitor_cfg_.environment_a_brake_m_s2 =
            inherit(m.environment.a_brake_m_s2, m.a_brake_m_s2);
        collision_monitor_cfg_.environment_hyst_m =
            inherit(m.environment.hyst_m, m.hyst_m);
        collision_monitor_cfg_.environment_recover_speed_m_s =
...
void DualArmServoLoop::loopMain() {
    ...
    while (running_) {
        ...
        // These are samples of THIS tick, not retained controller state.
        // Fault/read-invalid/freedrive branches can bypass computeServoTarget;
    ...
    sample.right_force_control = right_force_control_telemetry_;
        ...
        sample.safety_projection = safety_projection_telemetry_;
        {
            ...
            if (config_.safety.self_collision.enable && last_collision_verdict_.valid) {
                ...
                // Each pair also carries ITS OWN floor/slow band. The near list is
                // ordered by raw clearance, so a consumer that bands every pair against
                // the single self d_hard_m gets both the color and the "which parts are
                // colliding" answer wrong wherever the categories differ (the RB5's
                // structural intra-arm pair sits at ~23 mm against a 5 mm floor and
                // outranks an arm<->stand pair that IS violating its 40 mm floor).
                // nearPairHardFloorM/SlowBandM are the SAME functions
                // buildCollisionConstraints enforces with, so the two cannot drift.
...
void DualArmServoLoop::mergeAbcTelemetry(
    ...
    solve.follower_core_gate = abc.follower_core_gate;
    solve.follower_leash_gate = abc.follower_leash_gate;
    solve.follower_solve_failures = abc.follower_solve_failures;
    ...
    solve.follower_sample_velocity = abc.follower_sample_velocity;
    solve.follower_sample_acceleration = abc.follower_sample_acceleration;
    ...
    solve.follower_divergence_excused_count = abc.follower_divergence_excused_count;
    solve.follower_cmd_track_pos_m = abc.follower_cmd_track_pos_m;
    solve.follower_cmd_track_rad = abc.follower_cmd_track_rad;
    solve.follower_cmd_track_lag_ticks = abc.follower_cmd_track_lag_ticks;
    solve.follower_cmd_tracks = abc.follower_cmd_tracks;
...
void DualArmServoLoop::noteChunkFeedStall(ArmId arm_id, double age_sec, double timeout_sec) {
    const bool left = arm_id == ArmId::Left;
    bool& stalled = left ? left_chunk_feed_stalled_ : right_chunk_feed_stalled_;
    if (stalled) return;
    stalled = true;
    (left ? left_chunk_feed_stall_start_sec_ : right_chunk_feed_stall_start_sec_) =
        ChunkFrameReceiver::steadyNowSec();
    const std::uint64_t count = ++(left ? left_chunk_feed_stall_count_ : right_chunk_feed_stall_count_);
    std::cerr << "[WARN] chunk_follower " << toString(arm_id)
              << " chunk feed stalled: no frame for " << age_sec << " s (timeout "
              << timeout_sec << " s); holding at the reference until frames resume"
              << " (fallback_policy=hold, stall #" << count << ")\n";
}

void DualArmServoLoop::noteChunkFeedResume(ArmId arm_id) {
    const bool left = arm_id == ArmId::Left;
    bool& stalled = left ? left_chunk_feed_stalled_ : right_chunk_feed_stalled_;
    if (!stalled) return;
    stalled = false;
    const double start = left ? left_chunk_feed_stall_start_sec_ : right_chunk_feed_stall_start_sec_;
    std::cerr << "[INFO] chunk_follower " << toString(arm_id)
              << " chunk feed resumed after " << (ChunkFrameReceiver::steadyNowSec() - start)
              << " s of stall hold\n";
}

void DualArmServoLoop::noteChunkEngageWait(ArmId arm_id, double elapsed_sec) {
    // Under fallback_policy=hold the engage wait has no deadline; say so every 5 s.
    const bool left = arm_id == ArmId::Left;
    double& last = left ? left_chunk_engage_wait_last_log_sec_ : right_chunk_engage_wait_last_log_sec_;
    const double now = ChunkFrameReceiver::steadyNowSec();
    if (elapsed_sec < 5.0 || now - last < 5.0) return;
    last = now;
    std::cerr << "[WARN] chunk_follower " << toString(arm_id)
              << " still waiting for a chunk frame after " << elapsed_sec
              << " s (fallback_policy=hold: holding at the reference, no fault)\n";
}
...
ArmCommand DualArmServoLoop::applyChunkFollowerStage(
    ...
    abc.follower_core_gate = 1.0;
    abc.follower_leash_gate = 1.0;
    abc.follower_solve_failures = 0;
    ...
    abc.follower_cmd_track_pos_m = 0.0;
    abc.follower_cmd_track_rad = 0.0;
    abc.follower_cmd_track_lag_ticks = -1;
    abc.follower_cmd_tracks = false;
    ...
    // fallback_policy=hold (2026-09-04): the same strict shape as fault -- never chase a
    // stale target through the SMD -- but a LIVENESS loss (no first frame, feed stall)
    // holds at the reference and waits instead of latching. MEASURED the case that
    // asked for it: servo_log_20260904_020517.csv 85.6-87.1 s, the policy server
    // answered in ~800 ms with the GPU shared (3 openpi servers + 2 Isaac processes),
    // the runner was already re-emitting its last target as an explicit hold (command
    // age 1-2 ms the whole time), and the 1.5 s feed watchdog still latched
    // ChunkFollowerFault, which aborted the rollout and demanded ResetFault + re-init
    // + re-tare for a slow inference. A dead producer is the client deadman's case,
    // not this watchdog's. Divergence keeps latching under hold: that is the robot
    // leaving the plan, not the feed.
    const bool hold_policy = rf.fallback_policy == RuckigFollowerFallbackPolicy::Hold;
    const bool strict_policy = fault_policy || hold_policy;
    const bool was_active = follower->active();
    // THE HAND-OFF VELOCITY. Every path below that drops the follower and falls back to
    // the pose-track SMD used to seed that SMD at REST (SmdPoseTracker::reset zeroes the
    // state velocity), so an arm streaming at 145 mm/s stopped in ONE tick (-11,171
    // deg/s^2 on J5, servo_log_20260904_003600.csv 148.554 s) and then chased its
    // target from zero. Capture the follower's state velocity while it is still the
    // driver; the fallback seeds the SMD with it and decelerates on the SMD's own caps.
    const std::optional<Vec6> handoff_velocity =
        was_active ? follower->currentVelocity() : std::nullopt;
    ...
    const std::size_t fi = arm_id == ArmId::Left ? 0 : 1;
    const auto smd_fallback = [&]() {
        ...
        ForceControlTelemetry& ftel =
            fi == 0 ? left_force_control_telemetry_ : right_force_control_telemetry_;
        ...
        ArmCommand out = applyPoseTrackSmd(
            ...
            // An uncovered arm must never be cut by a stale gate.
            ftel.reference_strip_enabled ? (fi == 0 ? &left_force_gate_ : &right_force_gate_) : nullptr,
            &ftel.gate_removed_m,
            handoff_velocity ? &*handoff_velocity : nullptr,
            &contact_normal_[fi],
            &demand_m_s);
        // AN ABSOLUTE TARGET IS A SOURCE (2026-09-15): its demand is the raw target's
        // step rate, never the tracker's own motion. A raw Hold braking here is no
        // source at all.
        force_source_[fi] = command.mode == ControlMode::TcpPoseTarget
            ? ForceSource{ForceSourceKind::AbsoluteTarget, demand_m_s}
            : ForceSource{};
        applyPoseTrackWallFold(arm_id, smd_tracker, dt_sec, &out);
        return with_stage_telemetry(out);
    };
    // THE HOLD SOURCE STAGE (2026-09-15). A Hold under force control is a source with
    // ZERO demand: the pose it asks for is the hold source pose the coverage block
    // promoted (the last commanded TCP, moved along by the fold), and it BYPASSES the
    // pose-track SMD - a constant pose has nothing to smooth, and the SMD's goal rate
    // used to read the Hold's own yield back as stream demand (corr 0.997 with the
    // overlay velocity; the gate closed to 0.095 during a hand push,
    // servo_log_20260915_112545). The admittance law's own tau = m/b and the rate caps
    // are the only dynamics; the ROI/floor wall is applied to the hold pose in
    // foldForceDeviation.
    const auto hold_source_stage = [&]() {
        resetPreviewExecution(arm_id, "hold_source");
        output_smd->deactivate();
        smd_tracker->deactivate();
        resetChunkFollowerEngageWait(arm_id);
        log_transition();
        force_source_[fi] = ForceSource{ForceSourceKind::HoldPose, 0.0};
    ...
    if (rf.enable && chunk_frame_receiver_ &&
        ...
        resetChunkFollowerEngageWait(arm_id);
        if (command.compliant_hold) return hold_source_stage();
        if (smd_tracker->active()) {
            // The SMD brakes the raw Hold (release brake); the chunk plan stays
            // paused and expires on its own clock.
            return smd_fallback();
        }
        return with_stage_telemetry(command);
    }
    if (command.compliant_hold) {
        // A Hold under force control never engages or waits on the chunk plan (the
        // follower is inactive here; an active one was paused above).
        transition_reason = "hold source";
        return hold_source_stage();
    }
    ...
    // DOES THE ROBOT TRACK ITS COMMAND? This is the question every plan-vs-command
    // watchdog below (divergence, actual lead) actually needs answered, and it has to
    // be asked with the box's dead time in mind: the measured pose at this tick is the
    // response to a command sent ~22 ms ago, so the test is "does the measured pose
    // match ANY command sent within the last 50 ms" (CommandTrackingWindow). The
    // tolerance is half the lead budget: tighter than the lead gate it excuses, so the
    // excuse can never be looser than the thing it overrides. Same measured-vs-plan
    // side of the force overlay as `reference` (both nominal).
    control::CommandTrackingWindow<kCommandTrackingWindowTicks>& cmd_track_window =
        arm_id == ArmId::Left ? left_command_track_window_ : right_command_track_window_;
    cmd_track_window.record(reference);
    const control::CommandTrackingMatch cmd_track = cmd_track_window.match(
        actual_feedback_pose, 0.5 * rf.preview_max_actual_lead_m, 0.5 * rf.preview_max_actual_lead_rad);
    const bool robot_tracks_command = cmd_track.within;
    abc.follower_cmd_track_pos_m = cmd_track.pos_m;
    abc.follower_cmd_track_rad = cmd_track.ang_rad;
    abc.follower_cmd_track_lag_ticks = cmd_track.lag_ticks;
    abc.follower_cmd_tracks = robot_tracks_command;
    std::uint64_t& divergence_excused = arm_id == ArmId::Left
        ? left_divergence_excused_count_ : right_divergence_excused_count_;
    abc.follower_divergence_excused_count = divergence_excused;
    ...
    if (command_refused_recent && follower->active()) {
        ...
        if (delta_preview && rf.continuous_hold_resume) {
            // The safety/IK stage already kept the SENT command stationary.
            // Do not replay an unaccepted plan endpoint or repeatedly reset
            // the output filter while the refusal debounce is active.
            const bool entering = !follower->holdPaused();
            follower->holdAtSentReference(reference, safety_hold_now_sec);
            if (entering) {
    ...
    if (follower->active()) {
        ...
        if (age > rf.chunk_feed_timeout_sec) {
            ...
            transition_reason = fault_policy
                ...
                : hold_policy
                ? "feed timeout -> hold " + diag_seq_labels(active_diag)
            ...
            if (strict_policy) {
                ...
                if (fault_policy) {
                    ...
                    noteChunkFeedStall(arm_id, age, rf.chunk_feed_timeout_sec);
                }
                log_transition();
                return with_stage_telemetry(hold_at_reference());
            }
        } else {
            const bool hard_divergence =
                pos_err > kPoseTrackDivergenceHardPosTolM || ang_err > kPoseTrackDivergenceHardAngTolRad;
            if (pos_err > kPoseTrackReanchorPosTolM || ang_err > kPoseTrackReanchorAngTolRad) {
                ...
                //
                // (2026-09-06) ROBOT TRACKING ITS COMMAND is checked FIRST. Plan-vs-sent
                // is the lag of our own chain (follower core limits, output SMD, IK
                // throttle, projection rows) whenever the arm is on its command, and
                // that lag is neither a runaway nor something re-anchoring should
                // discard (it would drop the rotation the chain is about to deliver
                // and lunge). It is excused up to the HARD bound; only the hard bound
                // still latches. servo_log_20260906_123643.csv 120.0 s ended a healthy
                // 84 deg/s policy wrist turn on exactly this soft bound.
                if (!hard_divergence && robot_tracks_command) {
                    ++divergence_excused;
                    abc.follower_divergence_excused_count = divergence_excused;
                    ...
                    if (last_log_ns == 0 || now_ns < last_log_ns ||
                        ...
                        std::cout << "[chunk_follower] " << toString(arm_id)
                                  << " divergence excused: the robot tracks its command"
                                  << " (cmd-vs-actual " << cmd_track.pos_m * 1000.0 << " mm / "
                                  << cmd_track.ang_rad * 180.0 / M_PI << " deg at lag "
                                  << cmd_track.lag_ticks << " ticks); plan-vs-sent"
                                  << " pos_err=" << pos_err
                                  << " ang_err=" << ang_err
                                  << " is this chain's own lag, no re-anchor"
    ...
    abc.follower_lead_reanchor_unexplained_count = lead_reanchors_unexplained;
                    ...
                    if (strict_policy) {
                        ...
                        reason << toString(arm_id)
                               ...
                               << (hard_divergence ? " (hard bound)" : " (robot off its command)")
                               << " pos_err_m=" << pos_err
                               << " pos_tol_m=" << kPoseTrackReanchorPosTolM
                               << " pos_hard_m=" << kPoseTrackDivergenceHardPosTolM
                               << " ang_err_rad=" << ang_err
                               << " ang_tol_rad=" << kPoseTrackReanchorAngTolRad
                               << " ang_hard_rad=" << kPoseTrackDivergenceHardAngTolRad
                               << " cmd_vs_actual_m=" << cmd_track.pos_m
                               << " cmd_vs_actual_rad=" << cmd_track.ang_rad
                               << " cmd_lag_ticks=" << cmd_track.lag_ticks
                               << " " << diag_seq_labels(active_diag);
                        recordChunkFollowerFaultRequest(arm_id, reason.str());
                        log_transition();
                        return with_stage_telemetry(hold_at_reference());
    ...
    if (!follower->active()) {
        if (strict_policy) {
            ...
            if (!waiting) {
                ...
                std::cout << "[chunk_follower] " << toString(arm_id)
                          << " waiting for first chunk frame (fallback_policy=" << (fault_policy ? "fault" : "hold") << " "
                          << seq_labels(*submitted_wire_seq, *submitted_recv_seq) << ")\n";
            }
            const double elapsed_sec = now_sec - wait_start_sec;
            if (hold_policy) noteChunkEngageWait(arm_id, elapsed_sec);
            if (fault_policy && elapsed_sec > rf.engage_timeout_sec) {
                std::ostringstream reason;
                reason << toString(arm_id)
    ...
    // DIVERGENCE LEASH (2026-09-06): this tick's plan-vs-sent divergence (already in
    // abc from the watchdog above) ramps the plan clock down toward min_gate as it
    // approaches the soft bound, so the chain's lag is absorbed as time. Min-combined
    // with the safety gate: the plan advances at the rate of the most restrictive
    // cause, never the product of overlapping ones (the IK throttle shows up in both).
    const double safety_plan_gate =
    ...
    if (rf.plan_leash_enable && !rf.preview_execution.enable) {
        ...
        leash.full_m = rf.plan_leash_full_m;
        leash.full_rad = rf.plan_leash_full_rad;
        leash.min_gate = rf.plan_leash_min_gate;
    ...
    // THE (gate, direction) PAIR THE FOLLOWER CUTS ALONG, published from ONE place
    // (publishAdvanceGate): the curve's ratio along this tick's measured F_hat, or the
    // ROI/floor face's normal at gate 0 while that fold owns the slot. On the preview
    ...
    // acceleration AT THIS SAMPLE, so it tracks the jerk-limited profile itself
    // without lag and filters only what the profile does not carry. The legacy path
    // fed the chained end-state velocity through a low-pass at nf, which lagged the
    // plan by ~3a/wn^2 under acceleration (29 mm at 56.2 s of the 13:17 run) and then
    // hit the reseed snap.
    const bool profile_ff = rf.output_smd.profile_feedforward;
    const bool physical_derivatives = delta_preview &&
        (rf.fresh_chunk_replan || rf.continuous_hold_resume);
    const control::FollowerOutputKinematics sampled_output = physical_derivatives
        ? follower->outputKinematics() : control::FollowerOutputKinematics{};
    // Record the actual sampled derivatives even with profile feedforward off.
    // Endpoint velocities are a different signal and cannot replay this filter.
    abc.follower_sample_velocity = physical_derivatives
        ? std::optional<Vec6>(sampled_output.velocity) : std::nullopt;
    abc.follower_sample_acceleration = physical_derivatives
        ? std::optional<Vec6>(sampled_output.acceleration) : std::nullopt;
    const Vec6 follower_velocity =
        physical_derivatives ? sampled_output.velocity
            : (profile_ff ? follower->sampledVelocity() : follower->currentVelocity()).value_or(Vec6{});
    const Vec6 follower_accel =
        profile_ff ? (physical_derivatives ? sampled_output.acceleration
                                          : follower->sampledAcceleration().value_or(Vec6{}))
                   : Vec6{};
    if (delta_preview && rf.continuous_hold_resume && follower->holdPaused()) {
        // Keep the same force-reference/compose contract as the existing hold.
        // The unaccepted pre-filter state is never sent while IK refuses it.
        smoothed.tcp_target_stand = reference;
        abc.follower_output_smd_active = rf.output_smd.enable && output_smd->active();
    } else if (rf.output_smd.enable) {
        if (!output_smd->active()) {
            ...
            Vec6 seed_velocity = follower_velocity;
            if (physical_derivatives) {
                // The physical sample is expressed in the prefilter body;
                // reset() instead expects angular velocity in its seed pose.
                const Eigen::Vector3d omega =
                    math::rotationFromPose(reference).transpose() *
                    math::rotationFromPose(sampled_output.pose) *
                    Eigen::Vector3d(follower_velocity.rx, follower_velocity.ry, follower_velocity.rz);
                seed_velocity.rx = omega.x(); seed_velocity.ry = omega.y(); seed_velocity.rz = omega.z();
            }
            output_smd->reset(reference, seed_velocity);
            abc.follower_output_smd_reseeded = true;
        }
        smoothed.tcp_target_stand = output_smd->step(
            pre_filter, follower_velocity, dt_sec, profile_ff ? &follower_accel : nullptr,
            physical_derivatives);
        // An internal reseed (reference jumped past the reseed bounds) is a snap of
        // the emitted target; make it visible in the same column as the loop's own.
        if (output_smd->reseededLastStep()) abc.follower_output_smd_reseeded = true;
    ...
    force_source_[fi] = ForceSource{ForceSourceKind::ChunkFollower, diag.demanded_advance_m_s};
    ...
    //  (2026-09-06) The test itself moved up to where `reference` is formed and became
    //  dead-time tolerant (CommandTrackingWindow): a naive |sent_now - actual_now| grows
    //  with speed (1.4 rad/s x 22 ms = 1.8 deg) and would stop excusing exactly when the
    //  arm moves fast. `robot_tracks_command` is now shared with the divergence gate.
    ...
    if (lead_breach && robot_tracks_command) {
        ...
        if (lead_log_ns == 0 || now_ns < lead_log_ns ||
            ...
            std::cout << "[chunk_follower] " << toString(arm_id)
                      ...
                      << cmd_track.pos_m * 1000.0 << " mm / "
                      << cmd_track.ang_rad * 180.0 / M_PI
                      << " deg at lag " << cmd_track.lag_ticks
                      << " ticks); the lead is this chain's own lag, no re-anchor"
    ...
    abc.follower_core_gate = follower->coreGate();
    abc.follower_solve_failures = follower->diag().solve_failure_count;
...
ArmCommand DualArmServoLoop::applyDeltaTwistFollowerStage(
    ...
    if (command.compliant_hold) {
        // THE HOLD SOURCE (2026-09-15): zero demand, no SMD - see the Ruckig stage.
        transition_reason = "hold source";
        follower->deactivate();
        smd_tracker->deactivate();
        resetChunkFollowerEngageWait(arm_id);
        log_transition();
        force_source_[arm_id == ArmId::Left ? 0 : 1] = ForceSource{ForceSourceKind::HoldPose, 0.0};
        return with_stage_telemetry(command);
    }
    if (!rf.enable || !chunk_frame_receiver_ || command.mode != ControlMode::TcpPoseTarget ||
        !command.has_tcp_target || !kinematics_) {
    ...
    if (follower->active()) {
        ...
        if (age > rf.chunk_feed_timeout_sec) {
            ...
            if (pos_err > kPoseTrackReanchorPosTolM || ang_err > kPoseTrackReanchorAngTolRad) {
                if (strict_policy && divergence_explained_recent) {
                    follower->reanchor(reference);
                    ++reanchor_count;
                    ++divergence_reanchors;
                    abc.follower_reanchor_count = reanchor_count;
    ...
    if (!follower->active()) {
        if (strict_policy) {
            ...
            if (!waiting) {
                ...
                wait_start_sec = now_sec;
                std::cout << "[chunk_follower] " << toString(arm_id)
                          << " waiting for first delta chunk frame (fallback_policy=" << (fault_policy ? "fault" : "hold") << " "
                          << seq_labels(*submitted_wire_seq, *submitted_recv_seq) << ")\n";
            }
            const double elapsed_sec = now_sec - wait_start_sec;
            if (hold_policy) noteChunkEngageWait(arm_id, elapsed_sec);
...
ServoTarget DualArmServoLoop::computeServoTarget(
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        std::optional<Pose6D>& hold_nominal = hold_source_pose_[static_cast<std::size_t>(i)];

        tel = ForceControlTelemetry{};
        tel.enabled = config_.force_control.enable;
        const auto& reference_dp = overlay.deviation();
        const auto& reference_er = overlay.deviationRot();
        tel.reference_deviation_m = {reference_dp.x(), reference_dp.y(), reference_dp.z()};
        tel.reference_deviation_rad = {reference_er.x(), reference_er.y(), reference_er.z()};
        tel.reference_reset_count = force_reference_reset_count_[static_cast<std::size_t>(i)];
        {
            // THE RUNNING TOTALS ARE PUBLISHED EVERY TICK, covered or not (2026-09-15):
            // written only inside foldForceDeviation they read 0 on any tick the fold
            // did not run (uncovered, no target, a recovery brake) and then jumped back,
            // which made "how far has force moved this plan" a sawtooth in the CSV.
            const math::Vector3& absorbed = fc_left ? left_fc_absorbed_m_ : right_fc_absorbed_m_;
            const Eigen::Quaterniond& absorbed_r = fc_left ? left_fc_absorbed_r_ : right_fc_absorbed_r_;
            ...
            tel.source = forceSourceName(force_source_[static_cast<std::size_t>(i)].kind);
            tel.source_demand_m_s = force_source_[static_cast<std::size_t>(i)].demand_m_s;
        }
        ...
        // These execution guards must run BEFORE any nominal reference is stripped.
        // A frozen deviation is retained during coverage loss, but cannot be removed
        // from a hold target unless the downstream compose will put it back.
        if (covered && qsyncSettlingHoldActive(fc_arm)) {
            covered = false;
            reason = "queue-sync settling hold pins this arm's output - compliance resumes on track";
        }
        if (covered && eff.mode == ControlMode::TcpPoseTarget && !eff.has_tcp_target) {
            covered = false;
            reason = "no Cartesian target on this tick";
        }
        if (covered && eff.mode != ControlMode::Hold &&
            !servo_stream_armed_.load(std::memory_order_relaxed)) {
            covered = false;
            reason = "servo stream is not armed (no motion command yet)";
        }
        // A compliant Hold can arm the stream below when it latches its reference.
        ...
        if (eff.mode != ControlMode::Hold) {
            // A PLAN (or an absolute target) is being streamed: that stage is the
            // source and writes its own demand. Nothing to promote here.
            hold_nominal.reset();
            continue;
        }
        // A HOLD UNDER FORCE CONTROL IS A SOURCE WITH ZERO DEMAND (2026-09-15). Same
        // law as every other source; what it asks for is the pose the arm was last
        // commanded to, latched ONCE on entry and moved along by the fold every tick
        // (re-reading it every tick would make the source chase its own yield).
        if (!hold_nominal.has_value()) {
            ...
            // LATCHING UNDER LOAD IS FINE (2026-09-15). The old relatch guard kept a
            // SPRING from being anchored at the pushed pose; with k = 0 there is no
            // spring, the latch is at the COMMAND (no step), and the law yields from
            // wherever it latched. A hand already pushing simply starts to move the arm.
            ...
            std::cerr << "[INFO] force overlay " << toString(fc_arm)
                      << ": Hold is a force source, pose latched at the " << latched_from << " ["
    ...
    for (int i = 0; i < 2; ++i) {
        auto& tel = i == 0 ? left_force_control_telemetry_ : right_force_control_telemetry_;
        const auto& eff = i == 0 ? effective_command.left : effective_command.right;
        // Latch one decision for the reference and compose stages. In worker mode
        // queue-sync may change asynchronously; re-reading it between these stages
        // could strip an offset and then skip its inverse in the very same tick.
        tel.reference_strip_enabled = tel.coverage_reason == "covered" &&
            eff.mode == ControlMode::TcpPoseTarget && eff.has_tcp_target &&
            servo_stream_armed_.load(std::memory_order_relaxed);
    }
    ...
    const auto pause_chunk_follower_for_hold = [this](
        ...
        const double now_sec = ChunkFrameReceiver::steadyNowSec();
        if (built_config.controller == RuckigFollowerController::DeltaPreview &&
            built_config.continuous_hold_resume && kinematics_) {
            follower.holdAtSentReference(nominalOfEmitted(arm_id,
                kinematics_->computeTcpStand(arm_id, ctx.prev_sent_q_deg, ctx.mount)), now_sec);
        } else {
            follower.pauseForHold(now_sec);
        }
    ...
    if (isCartesianMode(effective_command.left.mode) || isCartesianMode(effective_command.right.mode) ||
        ...
        // ---- THE ROI / FLOOR FOLD ON THE FOLLOWER'S OWN TARGET (2026-09-04) ---------
        // The Tier-2 clamp above only sees the CLIENT's TcpPoseTarget; the chunk
        // follower then emits its own plan pose, which walked past the ROI face and
        // was cut back every tick by the projection rows downstream. MEASURED
        // (servo_log_20260904_005550.csv, right arm, 32.4-43.7 s at the y_min face):
        // the IK solved an 8-11 mm unreachable target every tick (raw jump 0.8-2 deg,
        // rate-limited to 0.3), the accel clamp passed ~0.006 deg of it, the projection
        // pushed 7-13 deg/s back -- q_sent alternated sign on J0/J1 on 91/81 % of ticks
        // (+/-0.004 deg, the loop's Nyquist rate) and the box q_ref carried it; the
        // plan meanwhile ran 13-25 mm past the face, so when the policy turned around
        // that lead discharged at 150-170 mm/s.
        // Here the emitted pose is clamped to the box (tip points included), the
        // clamp displacement is FOLDED into the plan the way the force fold books a
        // contact deviation (absorbOffset + output-SMD shift), and the plan's advance
        // INTO the face is gated (the same directional gate the force gate uses);
        // sliding along the face and backing away pass at full authority. The IK
        // target is therefore always reachable, no lead accumulates beyond the face,
        // and an escape starts from where the arm is at the plan's own speed. The
        // projection rows below stay as the backstop for everything else.
        for (int i = 0; i < 2; ++i) {
            ArmControlContext& ctx = arm_ctx[i];
            ArmCommand& track = arm_pose_track_command[i];
            const bool left_arm = i == 0;
            bool& fold_active = left_arm ? left_roi_fold_active_ : right_roi_fold_active_;
            const bool eligible =
            ...
            if (!eligible) {
                if (fold_active) {
                    fold_active = false;
                    roi_fold_normal_[static_cast<std::size_t>(i)].setZero();
                    publishAdvanceGate(ctx.arm);
                }
                continue;
            }
            const Pose6D emitted = track.tcp_target_stand;
            const Pose6D clamped = clampPoseToRoi(clampPoseToFloor(emitted));
            const math::Vector3 dp(clamped.x - emitted.x, clamped.y - emitted.y,
                                   clamped.z - emitted.z);
            const double dist = dp.norm();
            constexpr double kRoiFoldEpsM = 1e-7;
            const uint64_t now_ns = nowSteadyNs();
            if (dist > kRoiFoldEpsM) {
                ...
                // the same convention as the measured contact force (+F_hat).
                roi_fold_normal_[static_cast<std::size_t>(i)] = math::Vector3(dp / dist);
                uint64_t& count = left_arm ? left_roi_fold_count_ : right_roi_fold_count_;
                double& total = left_arm ? left_roi_fold_total_m_ : right_roi_fold_total_m_;
                uint64_t& started = left_arm ? left_roi_fold_started_ns_ : right_roi_fold_started_ns_;
                uint64_t& last_log = left_arm ? left_roi_fold_last_log_ns_ : right_roi_fold_last_log_ns_;
                if (!fold_active) {
                    fold_active = true;
                    started = now_ns;
                    total = 0.0;
                    ++count;
                    std::cerr << "[INFO] roi_fold " << toString(ctx.arm)
                              << " ENGAGED: follower target clamped " << dist * 1000.0
                              << " mm into the box; plan advance into the face gated (#"
                              << count << ")\n";
                    last_log = now_ns;
                }
                total += dist;
                publishAdvanceGate(ctx.arm);
                if (now_ns - last_log > 1'000'000'000ULL) {
                    last_log = now_ns;
                    std::cerr << "[INFO] roi_fold " << toString(ctx.arm) << " holding: "
                              << (now_ns - started) * 1e-9 << " s at the face, "
                              << total * 1000.0 << " mm folded so far\n";
                }
            } else if (fold_active) {
                fold_active = false;
                roi_fold_normal_[static_cast<std::size_t>(i)].setZero();
                publishAdvanceGate(ctx.arm);
                const uint64_t started = left_arm ? left_roi_fold_started_ns_ : right_roi_fold_started_ns_;
                const double total = left_arm ? left_roi_fold_total_m_ : right_roi_fold_total_m_;
                std::cerr << "[INFO] roi_fold " << toString(ctx.arm) << " released after "
                          << (now_ns - started) * 1e-9 << " s, " << total * 1000.0
                          << " mm folded; plan advance restored\n";
            }
        }
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (!tel.reference_strip_enabled) continue;
            ArmCommand& track = arm_pose_track_command[i];
            ...
            // Coverage, stream arming and queue-sync eligibility were latched before
            // reference conversion. The final safety/send gates still own the wire.
            ...
            foldForceDeviation(arm_id, force_source_[static_cast<std::size_t>(i)].kind,
                               arm_ctx[i].chunk_follower, tel);
            // FEED THE GATE TO THE PLAN, not just to the law: the overlay makes the arm
            // YIELD; only the gate stops the PLAN from walking further into what it is
            // yielding to. One publisher, one (gate, direction) pair, so the legacy
            // path and the preview path can no longer disagree about the direction.
            publishAdvanceGate(arm_id);
        }
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            // unless the solve was accepted -- a refused tick has its own hold path.
            {
                ...
                // A JOINT-MODE TICK IS A DISCONTINUITY, NOT A RELEASE. The ramp above
                // fades the filter out across a gate that clears; it must not bridge an
                // interval in which this arm was not solved at all (InitMotion, Hold, a
                // JointTarget jog). MEASURED 2026-09-04 servo_log_20260904_005550.csv,
                // right arm, 47.122 s: both arms had been IK-throttled against the ROI
                // face (engage 1.0) when a both-arm InitMotion took them 30-100 deg away
                // for 3.4 s. The first Cartesian tick after the Hold blended the FROZEN
                // filter state (the face pose) back in at alpha 0.41: q_before_safety
                // J2 -65.8 deg against a solution of -125.05, the velocity clamp shed
                // 58.95 deg, the accel clamp ran at its ceiling for 10 ticks and the
                ...
                engage = control::lowpassEngagementStep(
                    engage, gate_on && cartesian_this_tick,
                    config_.kinematics.ik.pinned_lowpass_release_sec, dt_sec);
                if (cartesian_this_tick && lp_hz > 0.0 && engage > 1e-3 &&
                    ctx.ik_pinned_lowpass_valid && dt_sec > 0.0) {
    ...
    // The pinned-unconverged IK low-pass lives in the Cartesian dispatch above and
    // never runs here, so without this its engagement and filtered joints would
    // FREEZE for the whole joint-mode interval and be blended into the first
    // Cartesian tick afterwards (the 2026-09-04 InitMotion resume kick, see the
    // block marked "A JOINT-MODE TICK IS A DISCONTINUITY").
    for (int i = 0; i < 2; ++i) {
        arm_ctx[i].ik_pinned_lowpass_engage = 0.0;
        arm_ctx[i].ik_pinned_lowpass_valid = false;
    }
...
ServoTarget DualArmServoLoop::applySafety(
    ...
    if (!safety_cons.empty() && !self_collision_hold &&
        ...
        // VERDICT HYSTERESIS (safety.projection_verdict_hold_sec). The verdict above
        // is "the projection cut something THIS tick"; a plan dithering on a face
        // flips it every few ticks (six Ok <-> RoiViolation toggles in 0.3 s measured
        // 2026-09-04). Hold the last projection verdict for a short window. Telemetry
        // only: the rows, the counters and the intervention stamp are already final.
        if (config_.safety.projection_verdict_hold_sec > 0.0) {
            const uint64_t now_ns = nowSteadyNs();
            ...
            if (projection_verdict) {
                ...
                held_projection_verdict_until_ns_ =
                    now_ns + static_cast<uint64_t>(config_.safety.projection_verdict_hold_sec * 1e9);
            } else if ((combined == SafetyVerdict::Ok ||
                        combined == SafetyVerdict::JointLimitClamped) &&
                       now_ns < held_projection_verdict_until_ns_) {
                combined = held_projection_verdict_;
            }
        }
...
bool DualArmServoLoop::stepFtPipeline(ArmId arm, const RobotState& state) {
    ...
    // ---- tool inertia from the COMMANDED trajectory (2026-09-15) ----------------
    // The sensor reads -m*a_com beside m*g and the box compensates neither. The
    // commanded chain is smooth and known; the measured joints are not (27 N p99 of
    // noise from a 9-sample fit, 2026-09-11). One FK per tick pushes the tool COM's
    // stand position into a ring; the central second difference centred
    // command_delay_ticks back is the acceleration the sensor's flange is executing
    // NOW (measured lag ~14 ticks: sent -> box reference -> arm -> F/T sample).
    {
        const FtInertiaCompensationConfig& ic = config_.force_torque.inertia_compensation;
        ToolComHistory& hist = tool_com_history_[left ? 0 : 1];
        const JointArray& q_cmd = left ? left_prev_sent_q_deg_ : right_prev_sent_q_deg_;
        bool pushed = false;
        if (ic.enable && kinematics_ != nullptr && finiteJointArray(q_cmd)) {
            const ArmMountConfig& mount = left ? config_.left_mount : config_.right_mount;
            const std::optional<Pose6D> fl = kinematics_->computeFlangeStand(arm, q_cmd, mount);
            if (fl.has_value()) {
                const math::Matrix3 r = math::rotationFromPose(*fl);
                const math::Vector3 p_flange(fl->x, fl->y, fl->z);
                hist.p[hist.head] = p_flange + r * pipe.toolComInFlange();
                hist.head = (hist.head + 1) % kToolComRing;
                if (hist.count < kToolComRing) ++hist.count;
                pushed = true;
            }
        }
        if (!pushed) hist = ToolComHistory{};   // a gap in the command chain: nothing to differentiate
        tel.inertia_valid = false;
        tel.com_accel_stand_m_s2 = {0.0, 0.0, 0.0};
        // Centre index c ticks back (c >= 1 so the stencil is central); needs c + 2 samples.
        const std::size_t c = static_cast<std::size_t>(std::max(1, ic.command_delay_ticks));
        if (pushed && hist.count >= c + 2 && c + 2 <= kToolComRing) {
            const auto at = [&](std::size_t back) {
                return hist.p[(hist.head + kToolComRing - 1 - back) % kToolComRing];
            };
            const double dt = 1.0 / std::max(1, config_.servo.rate_hz);
            const math::Vector3 a = (at(c - 1) - 2.0 * at(c) + at(c + 1)) / (dt * dt);
            if (a.allFinite() && a.norm() <= ic.max_accel_m_s2) {
                in.com_accel_stand = a;
                in.inertia_valid = true;
                tel.inertia_valid = true;
                tel.com_accel_stand_m_s2 = {a.x(), a.y(), a.z()};
            }
        }
    }
    ...
    {
        // fillTelemetry does not know the acceleration; keep this tick's decision.
        const bool inertia_valid = tel.inertia_valid;
        const std::array<double, 3> a = tel.com_accel_stand_m_s2;
        pipe.fillTelemetry(&tel);
        tel.inertia_valid = inertia_valid;
        tel.com_accel_stand_m_s2 = a;
    }
    return ok;
}
...
bool DualArmServoLoop::forceControlCovered(
    ...
    switch (command.mode) {
        ...
        case ControlMode::Hold:
            // A Hold under force control is ALWAYS a source (demand 0): the coverage
            // block latches the last commanded TCP as its pose and promotes it, which
            // is what makes a hand-push test possible without a policy running.
            return true;
        default:
            break;
    }
...
const control::AdmittanceOverlay* DualArmServoLoop::forceOverlayForReference(ArmId arm) const {
    ...
    if (!tel.reference_strip_enabled) return nullptr;
    return left ? &left_overlay_ : &right_overlay_;
}

Pose6D DualArmServoLoop::nominalOfEmitted(ArmId arm, const Pose6D& emitted_stand) const {
    const auto* overlay = forceOverlayForReference(arm);
    return overlay ? overlay->strip(emitted_stand) : emitted_stand;
}
...
void DualArmServoLoop::resetForceReferenceForInit(ArmId arm) {
    ...
    // compliance displacement. Its brake keeps that joint pose/velocity intact.
    // Retaining a Cartesian reference from the preceding episode would replay it
    // on no-op completion, or after tare/coverage recovery at the new joint pose.
    (left ? left_overlay_ : right_overlay_).reset();
    ...
    tool_com_history_[i] = ToolComHistory{};
    clearLatchedCartesianTarget(arm);
    auto ctx = armContext(arm);
    ctx.chunk_follower.deactivate();
    ctx.chunk_follower.setAdvanceGate(1.0, Eigen::Vector3d::Zero());
    ctx.chunk_follower.setPlanRateGate(1.0);
    ctx.follower_output_smd.deactivate();
    ctx.pose_track_smd.deactivate();
    (left ? left_delta_twist_follower_ : right_delta_twist_follower_).deactivate();
    (left ? left_chunk_submitted_recv_seq_ : right_chunk_submitted_recv_seq_) = chunk_frame_cache_recv_seq_;
    resetChunkFollowerEngageWait(arm);
    pending_collision_fold_[i] = PendingCollisionFold{};
    hold_fold_emitted_pose_[i].reset();
    collision_fold_active_[i] = false;
    (left ? left_roi_fold_active_ : right_roi_fold_active_) = false;
    (left ? left_command_track_window_ : right_command_track_window_).clear();
    ++force_reference_reset_count_[i];
    std::cout << "[force_reference_reset] mono_ns=" << nowSteadyNs()
              << " arm=" << toString(arm) << " reason=fresh_init_motion count="
              << force_reference_reset_count_[i] << '\n';
}

void DualArmServoLoop::prepareForceOverlayInput(ArmId arm) {
    const std::size_t i = arm == ArmId::Left ? 0 : 1;
    ...
    // TELEMETRY KEEPS THE RAW (deadzoned) MEASUREMENT, so the published/logged wrench
...
void DualArmServoLoop::publishAdvanceGate(ArmId arm) {
    const bool left = arm == ArmId::Left;
    const std::size_t i = left ? 0 : 1;
    auto ctx = armContext(arm);
    const ForceControlTelemetry& tel = left ? left_force_control_telemetry_ : right_force_control_telemetry_;
    const control::ForceGate& gate = left ? left_force_gate_ : right_force_gate_;
    const bool roi_owns = left ? left_roi_fold_active_ : right_roi_fold_active_;
    if (roi_owns) {
        // The geometric wall owns the slot: no plan advance into the face, whatever
        // the force says. The law still yields to the contact through compose, and
        // the fold still books it.
        ctx.chunk_follower.setAdvanceGate(0.0, roi_fold_normal_[i]);
        return;
    }
    // An uncovered arm must not be cut by a stale gate: g = 1, no direction.
    if (!tel.reference_strip_enabled) {
...
const char* DualArmServoLoop::forceSourceName(ForceSourceKind kind) {
    switch (kind) {
        case ForceSourceKind::ChunkFollower: return "chunk_follower";
        case ForceSourceKind::HoldPose: return "hold";
        case ForceSourceKind::AbsoluteTarget: return "absolute";
        case ForceSourceKind::None: break;
    }
    return "none";
}
...
bool DualArmServoLoop::applyForceOverlay(ArmId arm, const RobotState& state, Pose6D* target) {
    ...
    *target = composed;
    ...
    tel.enabled = config_.force_control.enable;
    tel.covered = true;
    tel.source = forceSourceName(force_source_[i].kind);
    tel.source_demand_m_s = force_source_[i].demand_m_s;
    tel.contact_normal_stand = {contact_normal_[i].x(), contact_normal_[i].y(), contact_normal_[i].z()};
    const math::Vector3& d = overlay.deviation();
    const math::Vector3& dr = overlay.deviationRot();
    // Report the transform actually written, including sub-micrometre offsets.
    // compose() adds every nonzero translation and rotates at >= 1e-9 rad.
    tel.compose_applied = !d.isZero(0.0) || dr.norm() >= 1e-9;
...
void DualArmServoLoop::foldForceDeviation(ArmId arm, ForceSourceKind source,
                                          control::CartesianChunkFollower& follower,
                                          ForceControlTelemetry& tel) {
    const bool left = arm == ArmId::Left;
    const std::size_t i = left ? 0 : 1;
    control::AdmittanceOverlay& overlay = left ? left_overlay_ : right_overlay_;
    std::optional<Pose6D>& hold_pose = hold_source_pose_[i];
    ...
    // THE FOLD IS STRUCTURAL (2026-09-15): k = 0 by construction, so the hand-off is
    // always a change of variables (see AdmittanceOverlay::dropDeviation). WHO TAKES
    // IT is the SOURCE. The sink must be a chain that carries a displacement forward
    // on its own; a producer that re-issues an absolute target every tick would
    // overwrite the fold and the command would square-wave by one tick's travel.
    const char* sink = nullptr;
    switch (source) {
        case ForceSourceKind::ChunkFollower:
            if (!follower.active() || follower.holdPaused()) {
            ...
            sink = "chunk_follower";
            break;
        case ForceSourceKind::HoldPose:
            if (!hold_pose.has_value()) {
                tel.fold_sink = "declined: Hold has no source pose";
                return;
            }
            sink = "hold";
            break;
    ...
    if (source == ForceSourceKind::HoldPose) {
        // The hold source pose becomes the composed pose: same pivot (the TCP), same
        // displacement, so next tick's nominal IS this tick's command.
        //
        ...
        // the COMMAND built from the nominal, but the nominal itself kept walking out
        // of the box with the hand. MEASURED servo_log_20260907_021834.csv, right arm,
        // hand-guide: at y_min (610-616 s) the nominal was pushed ~130 mm past the
        // face while the command sat on it; the operator then pushed the other way at
        // 17-35 N for 2.2 s (~108 mm of hand travel) and the command did not move,
        // gave up, pushed again and got it moving only after 25 mm more -- the whole
        // excursion had to be unwound first. So: clamp the pose to the box and the
        // floor here, and let the part the box refuses vanish instead of being banked.
        // A reversal then moves the command on its first tick, which is what a wall is.
        const Pose6D composed = overlay.compose(*hold_pose);
        ...
        *hold_pose = walled;
        ...
        double& total = left ? left_hold_source_wall_refused_total_m_
                             : right_hold_source_wall_refused_total_m_;
        if (refused_m > 1e-9) {
            uint64_t& last_log = left ? left_hold_source_wall_refused_log_ns_
                                      : right_hold_source_wall_refused_log_ns_;
            ...
            if (first || now_ns - last_log > 1'000'000'000ULL) {
                ...
                std::cerr << "[INFO] force fold " << toString(arm)
                          << ": Hold source pose walled at the ROI/floor, refused "
        ...
        PendingForceFold& pf = pending_force_fold_[i];
        if (pf.valid) {
...
void DualArmServoLoop::resyncArmAfterFreedrive(ArmId arm_id, const RobotState& state) {
    ...
    tool_com_history_[arm_id == ArmId::Left ? 0 : 1] = ToolComHistory{};
...
DualArmCommand DualArmServoLoop::applyInitMotionSequencer(
    ...
    const auto reanchor_selected_to_measured = [&](bool request_left, bool request_right) {
        ...
        // ...but only when the gap is NOT a tracking residual. MEASURED 2026-09-04
        // (servo_log_20260904_005550.csv 43.914 s, both arms; _020517.csv 4.84 s and
        // 62.0 s): the brake had taken the arms to 0.2-0.5 deg/s, the command led the
        // encoder by 0.009-0.027 deg per joint, and this snap stepped every joint by
        // that residual in ONE tick - 4,683 (L J0) / 6,693 (R J4) deg/s^2 on the hold
        // tick, the largest acceleration of the whole InitMotion. Below
        // command_reanchor_deg the stream stays where it was and the joint-target SMD
        // closes the residual on its way to the first waypoint (it starts from the
        // last SENT velocity, see TrajectoryFilter::observeSentTarget). At or above it
        // the gap is a real displacement and the snap is the safe choice.
        const double reanchor_deg = config_.safety.init_motion_planner.command_reanchor_deg;
        const auto gap_deg = [](const JointArray& a, const JointArray& b) {
            double gap = 0.0;
            for (int i = 0; i < kDof; ++i) gap = std::max(gap, std::abs(a[i] - b[i]));
            return gap;
        };
        std::lock_guard<std::mutex> lock(state_mutex_);
        if ((request_left || freeze_other_arm) && finiteJointArray(current_left_q) &&
            (reanchor_deg <= 0.0 || !finiteJointArray(left_prev_sent_q_deg_) ||
             gap_deg(current_left_q, left_prev_sent_q_deg_) >= reanchor_deg)) {
        ...
        if ((request_right || freeze_other_arm) && finiteJointArray(current_right_q) &&
            (reanchor_deg <= 0.0 || !finiteJointArray(right_prev_sent_q_deg_) ||
             gap_deg(current_right_q, right_prev_sent_q_deg_) >= reanchor_deg)) {
    ...
    const auto process_exec = [&](InitMotionExec& ex, PlannerRequester requester,
        ...
        if (fresh_request && (request_left || request_right)) {
            ...
            if (request_left) {
                resetForceReferenceForInit(ArmId::Left);
                armAutoTareAfterInit(ArmId::Left);
            }
            if (request_right) {
                resetForceReferenceForInit(ArmId::Right);
                armAutoTareAfterInit(ArmId::Right);
            }
            const JointArray target_left =
