...
nlohmann::json ftJson(const FtTelemetry& t) {
    return {
        ...
        {"inertial_sensor_at_sro", wrenchJson(t.inertial_sensor)},
        {"com_accel_stand_m_s2", t.com_accel_stand_m_s2},
        {"inertia_valid", t.inertia_valid},
...
nlohmann::json forceControlJson(const ForceControlTelemetry& t) {
    return {
        ...
        {"source", t.source},
        {"source_demand_m_s", t.source_demand_m_s},
        {"contact_normal_stand", t.contact_normal_stand},
...
nlohmann::json selfCollisionManifestJson(
    ...
    return {
        ...
        {"pika_gripper_base_meshes", stringArrayJson(m.pika_gripper_base_meshes)},
...
std::string StatePublisher::serializeSnapshot(const ServoSnapshot& snapshot) const {
    ...
    const auto preview_json=[](const PreviewExecutionTelemetry& p) {
        nlohmann::json result{{"enabled",p.enabled},{"active",p.active},{"status",p.status},
            {"sample_time_ns",p.sample_time_ns},{"epoch",p.epoch},{"plan_id",p.plan_id},
            {"source_wire_seq",p.source_wire_seq},{"source_recv_seq",p.source_recv_seq},
            {"backlog_sec",p.backlog_sec},{"rate",p.rate},{"plan_age_sec",p.plan_age_sec},
            {"accepted_position_error_m",p.accepted_position_error_m},
    ...
    message["safety_verdict"] = toString(snapshot.safety_verdict);
    {
        ...
        if (snapshot.self_collision_mesh && !snapshot.self_collision_near_pairs.empty()) {
            ...
            for (const auto& p : snapshot.self_collision_near_pairs) {
                ...
                entry["a_brake_m_s2"] = p.a_brake_m_s2;
