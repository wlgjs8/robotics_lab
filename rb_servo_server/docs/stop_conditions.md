...
| C | Real-mode config guard weakened: a commit lets real motion proceed under a read-only config (`servo.send_servo_commands: false`, `cartesian_control.allow_in_real: false`) — i.e. G5 (real-mode config guard) no longer fail-closed. Real motion is config-driven, not env-gated (`RB_ALLOW_REAL_*` retired). | `./build/rb_servo_server --config config/local/stack_real_readonly.yaml` sending real `servo_j`/Cartesian without explicit config opt-in + commit hash. Per loop context constraint, this must never happen; if it does, the loop is in a state the architect/critic process cannot self-correct. |
