...
bool testStatePublisherSerializesServoSnapshotSchema() {
    ...
    const char* top_keys[] = {
        ...
        "cartesian_control_snapshot", "kinematics_snapshot",
...
bool testFreedriveExitResyncsHeldTargetToTaughtPose() {
    rb_servo::DualArmConfig cfg = rbpodoPhysicalRealConfig();
    configureCartesianLoopTest(&cfg);
    cfg.servo.allow_freedrive = true;
    ...
    // would; the held target must resume at the taught pose, not the
    // pre-teaching one.
...
int main() {
    ...
    if (!testFreedriveExitResyncsHeldTargetToTaughtPose()) return 1;
