...
| `cm_bridge/` | (P0: docs only) | **Combined-stack bridge (in design)**: will route the policy chunk stream through `submodules/controller-manager` (servo_j LPF-off on firmware 26071103, qsync queue regulation, force control) while keeping robotics_lab's CollisionMonitor as the pre-controller fail-closed gate and the existing wire contracts toward policy_runner/rb_gui. Streaming ingress decision: controller-manager `FollowUnit` (`/chimpanzee/<side>/cmd/follow`, per-period flange-frame deltas, REPLACE-at-boundary). See `cm_bridge/docs/design.md`. | `cm_bridge/README.md`, `cm_bridge/docs/design.md` |
| `submodules/controller-manager/` | C++17 / ROS 2 (submodule, READ-ONLY) | Company controller stack (per-arm RT threads, qsync queue-fill FLL, servo_j t1=2ms/t2=21ms/gain=1/alpha=10, firmware gate 26071103). Never edited here; updates via pin bump + SILS gate. | pinned SHA in `.gitmodules` |
...
| state out ← `rb_servo_server` | UDP JSON fanout | 50356 (scope), 50366 (viser GUI), 50376 (`make run` policy_runner), 50378 (external flow-infer), 50386 (spare; the camera wrist-fusion consumer was removed with the head-stereo pipeline) | `network.state_pub_endpoints` list |
...
| camera metadata | ZMQ PUB | 5600 | topics `camera.health` + bundle groups: `camera.bundle` (legacy full-rig), `camera.bundle.policy` (both wrists, real_policy gate), `camera.bundle.stereo` (head color, head rig only; feeds the rb_gui head view panel), `camera.bundle.wrist_left`/`.wrist_right` (per-side wrist RGB-D so one dead camera cannot block the healthy side). Wrist-only rig: `make cam-up-wrists` (dual_realsense_d405.yaml, no head D435) |
...
4. **State publication is a fanout, not a single port.** Current stack configs publish to `:50356` (scope), `:50366` (viser GUI), `:50376` (`make run` policy_runner), `:50378` (external flow-infer), and `:50386` (spare — its camera-side consumer was removed with the head-stereo pipeline) via `state_pub_endpoints`.
