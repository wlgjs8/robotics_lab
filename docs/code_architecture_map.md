...
7. **Unified collision URDF is now TRACKED here (changed 2026-09-02).** The self-collision `CollisionMonitor` loads `safety.self_collision.mesh.unified_urdf`, which since the RB3-730E -> RB5-850E swap is `rb_servo_server/descriptions/urdf/dual_rb5_850e_ver3.urdf` — git-tracked in this repo, generated from upstream `mo_robot_descriptions` `dual_rb5_850e_ver2.urdf` by `rb_servo_server/tools/make_rb5_850e_urdfs.py`. It is generated rather than copied because upstream's ver2 needs three safety-relevant corrections (non-convex collision shells, a `+/-179.9 deg` elbow bound against the catalog's `+/-165`, and a stand box approximation covering only 77% of the real stand). The generator's `--check` mode re-derives and diffs, so drift between script and committed URDF is detectable. The single-arm `rb_servo_server/descriptions/urdf/rb5_850e.urdf` (`kinematics.urdf`) and the rb_gui display model `rb5_850e_pika_articulated.urdf` come from the same generator. Previously this entry read "untracked external asset", which described the RB3 arrangement.
