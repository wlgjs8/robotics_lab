...
- Self-collision is a server-side async URDF-mesh guard (`rb_servo_server` `CollisionMonitor`, ~33 geoms / 337 pairs) run off the 2 ms servo path; `applySafety` reads the latest verdict and applies a velocity barrier. Any compiled capsule branch is an implementation remnant, not an accepted active safety profile.
