...
2. **Milestone is now physical bring-up; older circle-benchmark code is a separate evidence lane.** The current milestone is rbpodo pgmode-real physical bring-up — real motion has been exercised under supervision (`docs/runbooks/rbpodo_real_physical_circle.md`). A large volume of `GOAL.md`, `scripts/rbpodo_*`, `configs/rbpodo_circle_ablation/*`, and many runbooks still concentrate on the rbpodo controller-`pgmode`-simulation circle-tracking benchmark (ACKON500 / 500 Hz); that is controller-reference (`tcp_ref_stand`) evidence and is a **separate category** from physical-real (`tcp_actual_stand`) evidence. Don't conflate an ACKON500 controller-sim pass with a physical-real pass.
...
8. **Doc precedence.** Per `AGENTS.md`, when docs conflict the order is `AGENTS.md` → root `README.md` (KO; `README.en.md` is the English original) → `docs/architecture.md` → contract docs → component READMEs. `REVIEW.md` and `GOAL.md` are snapshots; `docs/current_review.md` is a redirect to `REVIEW.md`; `docs/archive/**` is audit-only.
