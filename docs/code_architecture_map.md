...
and `TcpLinearMove` compute in every run mode when the tracked stack config opens the
...
- Self-collision is a server-side async URDF-mesh guard (`rb_servo_server` `CollisionMonitor`; on the RB5-850E pair 57 geoms / 1062 curated pairs: left-right 196, arm-stand 702, intra-arm 108, gripper-gripper 9 inside left-right's count until 2026-09-04, when they became their own class) run off the 2 ms servo path on its own SCHED_FIFO thread (`monitor_core` 4, event-driven, one evaluation per submitted target); `applySafety` reads the latest verdict and builds per-pair velocity-damper rows into one Gauss-Seidel solve with the floor/ROI/reach rows. Any compiled capsule branch is an implementation remnant, not an accepted active safety profile.
- Force-control v2 is live in `rb_servo_server`: rbpodo reads raw external F/T channels, `FtPipeline` applies the measured CM-derived sensor/tool calibration and tare/gravity compensation, and the force overlay publishes per-arm `force_torque`/`force_control` telemetry. A client `force_control` object remains rejected because laws are config-owned.
- J3 is exactly `[-150 deg, +150 deg]` in both tracked stacks, the joint-limit barrier, and the URDF/Pinocchio model.
...
6. **GUI client-side execution lock is retired; server real-motion authority is config-driven.** `rb_gui` `safety.py` (`blocked_reason`, `tcp_command_disabled_reason`) no longer blocks real-mode commands or gates on a mode/backend match, and the old `RB_GUI_ENABLE_*` opt-in env flags are gone — controls emit in every run mode. The legacy **server** `RB_ALLOW_REAL_ROBOT/MOTION/CARTESIAN` env gates were also removed from the runtime (`config.cpp` "Real/sim env gates retired"); real motion is now decided by the tracked stack config (`cartesian_control.allow_in_real: true`) + the mode-independent server safety layers. Operator supervision remains physical procedure. Don't read "GUI lock removed" as "real-motion authority removed" — `rb_servo_server` still makes the final allow/deny decision.
7. **Unified collision URDF is now TRACKED here (changed 2026-09-02).** The self-collision `CollisionMonitor` loads `safety.self_collision.mesh.unified_urdf`, which since the RB3-730E -> RB5-850E swap is `rb_servo_server/descriptions/urdf/dual_rb5_850e_ver3.urdf` — git-tracked in this repo, generated from upstream `mo_robot_descriptions` `dual_rb5_850e_ver1.urdf` by `rb_servo_server/tools/make_rb5_850e_urdfs.py`. **Upstream ver1, not ver2, since 2026-09-06**: the two stands mount each arm 15.00 mm apart along the 45 deg plate (21.2 mm of arm-to-arm Y separation) and this cell has ver1 — measured by tape on the stand's Y width (507 vs 521 mm) and by a right-arm stand contact far from its own mount (0.50 mm off the true surface under ver1, 7.00 mm under ver2). `left_mount`/`right_mount` in both stack configs carry the same transform and are asserted equal by `rb_gui`. It is generated rather than copied because upstream needs three safety-relevant corrections (non-convex collision shells, a `+/-179.9 deg` elbow bound against the supported `+/-160` (the catalog's mechanical `+/-165` is itself unusable — the controller's self-collision detector trips at `|J3| ~ 161`, measured 2026-09-16; see `docs/joint_range_policy.md`), and a stand collision that ver1 ships only as a raw 38k-triangle non-convex mesh — replaced by 20 CoACD hulls covering 100% of the surface at mean 1.41 mm over-approximation). The generator's `--check` mode re-derives and diffs, so drift between script and committed URDF is detectable. The single-arm `rb_servo_server/descriptions/urdf/rb5_850e.urdf` (`kinematics.urdf`) and the rb_gui display model `rb5_850e_pika_articulated.urdf` come from the same generator. Previously this entry read "untracked external asset", which described the RB3 arrangement.
8. **The real Servo J profile is LPF-OFF, and the control box runs firmware v8.7.3.** The tracked `rb_servo_server/config/stack_real.yaml` pins `servo_alpha: 10.0` on both arms (script-level; effective `1.0` = controller inner LPF off) together with `servo.io_model: worker` and `queue_sync.enable: true, target_fill: 5`. Any doc that names `servo_alpha: 1.0` as "the tracked real profile" or `10.0` as "a diagnostic profile" predates 2026-08-25 and is stale — the jerk/jitter that justified the LPF was an upstream pure-pursuit command ripple, since fixed. Firmware **v8.7.3** replaced the box's latest-queue with a FIFO, so `box delay = RBACK queue fill + 1 tick` and the host must regulate the depth; v8.6.1-era latency numbers are not comparable and a software stop now queues behind the backlog. Normative: `docs/servo_backend_contract.md` → "Servo J Streaming Profiles" and "Control-Box Command Queue (firmware v8.7.3)"; procedure: `docs/runbooks/box_latency_offline.md`.
9. **Worker interpolation is not yet the accepted real profile.** The implementation and CSV telemetry exist, but `stack_real.yaml` keeps `servo.worker_setpoint_interpolation: false` pending an isolated supervised hardware A/B. Qsync settling hold is active.
9. **Doc precedence.** Per `AGENTS.md`, when docs conflict the order is `AGENTS.md` → root `README.md` (KO; `README.en.md` is the English original) → `docs/architecture.md` → contract docs → component READMEs. `REVIEW.md` is a snapshot; `docs/current_review.md` is a redirect to `REVIEW.md`; `docs/archive/**` is audit-only.
