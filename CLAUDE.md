...
- **`cm_bridge`** (P0, design only) + **`submodules/controller-manager`** (READ-ONLY git submodule, company controller stack): the in-progress combined-stack path. controller-manager is consumed at a process boundary and NEVER edited in-tree (changes go upstream as PRs; updates = pull -> SILS gate -> pin bump commit). Design and phase plan: `cm_bridge/docs/design.md`. During the transition `rb_servo_server` remains the active controller. `submodules/mo_forcecontroller` and `mo_grippers` were removed 2026-08-16.
...
Never commit inside `submodules/controller-manager` (read-only submodule; see `submodules/README.md`). Work only on the assigned task; prefer small reviewable changes; keep configs strict and fail-closed; update tests, docs, and acceptance scripts together with behavior. Never fake rbpodo, Pinocchio, RealSense, SpaceMouse, or camera APIs — if a dependency is missing, report it and don't claim the gate passed. Don't claim C++/Pinocchio/Cartesian runtime acceptance passed unless the command was actually run.
