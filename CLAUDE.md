...
make cloud-up / cloud-down            # wrist (D405) point-cloud publisher for the rb_gui viser scene
```
GUI: `http://127.0.0.1:8080`. Docker is used ONLY for `camera_server`; everything else runs natively. Camera lifecycle is managed by the `cam-*` targets (`cam-up`/`cam-up-wrists`/`cam-down`/`cam-status`); override the rig with `make cam-up CAMERA_CONFIG=/app/config/<rig>.yaml`. The rb_gui wrist point cloud is a SEPARATE host process (`make cloud-up`, `camera_server/stereo_worker/wrist_cloud_worker.py`): it reads the D405 depth bundles off `ZMQ 5600` + the shm ring and publishes `stereo.wrist` on `ZMQ 5601`, which `rb_gui/rb_servo_gui/pointcloud_receiver.py` subscribes to. No model or GPU — the D405 gives depth directly. It was restored 2026-08-27 after commit `3287da8` deleted the whole `stereo_worker` along with the Fast-FoundationStereo submodule; the head-stereo/box-detect/fusion half of that worker went with FFS and is NOT restored, but the wrist path never depended on it. Flow-matching training runs natively on the GPU server with `python3 -m policy_runner flow-train`.
