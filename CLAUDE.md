...
`robotics_lab` is a dual-arm RB5-850 integration workspace (RB3-730 until 2026-09-02). Before editing, read the current source-of-truth docs (they win over any historical prompt/planning file):
...
Real-motion execution authority is owned by **the tracked stack config + the mode-independent safety layers**:
...
Other invariants: never reintroduce bool-only backend results (preserve `BackendResult<RobotState>`, `SendServoJResult`, `BackendErrorKind`, `BackendTiming`, `FaultContext`); don't parse error strings when structured fields exist; force control is a live subsystem again (v2, CM-referenced) — `force_control:` / `force_torque:` ARE config sections. Since 2026-09-15 there is ONE source-agnostic law on the force VECTOR in the stand frame, `m*v' + b*v = (|F| - rest_force_n)+ * F_hat`, k = 0 by construction, rotation `rigid`: at or below `rest_force_n` nothing moves in any direction, above it the arm yields ALONG the measured force at `(|F| - rest)/b` and STAYS (the deviation is folded into the source's plan every tick, so the plan is always where the arm actually is — yield-and-stay; a stream spring `k = 400` was tried on 2026-09-10 and reverted the same day because it returns the arm instead of leaving it where it was dragged). The contact force is DECLARED in two numbers (`force_gate.peak_force_n` 24 / `rest_force_n` 20 + `peak_vel_mm_s` 8 since 2026-09-15 pm — 12 / 10 / 4 before — which DERIVES `b` = 500 N·s/m; `b` may not be typed; the wrench VECTOR is low-passed on TWO bandwidths since 2026-09-15 night: the GATE reads `wrench_filter_hz` 3 Hz because the force the sensor reads while the arm moves is the arm's own 15-24 Hz ringing, not contact, and the LAW reads `law_filter_hz` 25 Hz so a real impact still yields within milliseconds — fed the 3 Hz vector the law slept through a 40-84 N intermittent contact): CM 0049's exponential gate, judged on |F| and cutting only the source's advance INTO `F_hat` along the one `contact_normal` the loop publishes, crosses the law's yield line at `peak_force_n` for every demand. Hold and stream are SOURCES of that one stage (`chunk_follower` / `hold` / `absolute`; a Hold under force control is always a covered zero-demand source), not laws. DELETED keys are refused at load, never ignored: `force_gate.max_force_n`, `max_torque_nm` and the sustained-contact `stream_*` channel (2026-09-11), and `force_control.stream`, `hold`, `hold_compliance`, `hold_engage_force_n`, `hold_release_force_n`, `hold_relatch_max_force_n`, `fold_deviation` plus a typed `law.translation.{b,k,mode,ref_force}` or a non-`rigid` `law.rotation` (2026-09-15) — see `AGENTS.md` § Force Control; `servo.io_model: worker` is a supported real-mode path (the mock-only refusal was retired) — it is what control-box queue sync runs on, because holding each box's queue at a fixed depth requires the arm to own its own send cadence and one loop has a single period for two boxes with two clocks. The stand-frame floor plane (`safety.floor_constraint`) applies in EVERY run mode when enabled (no env/mode gate), covers all motion primitives at the final joint-level safety gate, requires `kinematics.enable`, and its runtime lowering via the leaseless `SetSafetyFloorZ` command is bounded to the config `[runtime_min_z_m, runtime_max_z_m]`; `monitor_only` is never a real-motion posture. There are exactly TWO stack configs and nothing else is a launch surface: `rb_servo_server/config/stack_real.yaml` and `rb_servo_server/config/stack_sim.yaml`. **Do not create `config/local` variants** (`AGENTS.md`) — change one reviewed setting at a time in the appropriate tracked config so the runtime profile stays visible and auditable. Two concrete reasons beyond reviewability: a config one directory deeper silently mis-resolves every relative path, because `config.cpp` resolves them against CWD, then the config's own directory, then that directory's parent (measured 2026-08-26: `kinematics.urdf`, `unified_urdf` and `package_dirs` all missed, and the failure surfaces only as "file does not exist"); and `config/local/` was never actually in `.gitignore`, so a variant carrying real-motion enables was one `git add` away from being committed. New rbpodo configs use canonical Servo J fields `servo_t1_sec` / `servo_t2_sec` / `servo_gain` / `servo_alpha` (not `servo_time_sec` / `servo_lookahead_sec` / `servo_acc`); `servo_t1_sec: 0.002` at the supported 500 Hz.

J3/elbow is fixed to the FITTED ARM's supported range in tracked safety limits, the
joint-limit barrier, IK, examples, and runbooks — `[-160 deg, +160 deg]` on the
RB5-850E since 2026-09-16, `[-150, +150]` on the RB3-730E it replaced. On the RB5 the
Rainbow catalog's mechanical range is `+/-165`, but the controller's own
self-collision detector latches at `|J3| ~ 161` (measured 161.10 / 161.55 deg,
`op_stat_self_collision` -> code 1005, no joint-limit code), so the usable bound is
smaller than the mechanical one. The bound is a property of the arm, not a tuning
knob: never restore a wider site margin, never widen the URDF to hide an unreachable
Cartesian target, and never move one layer alone (all of URDF, safety clamp, barrier
and `kKnownArmRanges` carry the same number).
...
The gripper's static base is checked as THREE convex pieces
(`safety.self_collision.mesh.pika_gripper_base_meshes`, flange / housing / guide), not one
hull, since 2026-09-18; the singular `pika_gripper_base_mesh` is refused at load. One hull
over that base has to span the 215.00 mm LM guide rail and the Ø70 flange at opposite ends
of the part and claims the cone between them — 4.16x the true volume, and measured p50 5.4
/ max 10.0 mm of phantom on the logged gripper<->gripper holds, worst exactly where one
gripper's rail tip faces the other's housing flank (true 13.9 mm reported as 4.9). The
pieces are generated by `make_pika_tool_meshes.py`, split by connected component, and
their union is asserted to contain the source mesh, so the split is a refinement and never
a hole. NO barrier margin moved with it and none may be widened to "compensate"; see
`docs/reference/pika_tool_geometry.md`.
...
make cam-up-wrists    # wrist-only rig: dual D405 without the head D435 (+ the wrist point-cloud worker)
make cloud-up / cloud-down            # that worker on its own (cam-up-wrists starts it, cam-down stops it)
```
GUI: `http://127.0.0.1:8080`. Docker is used ONLY for `camera_server`; everything else runs natively. Camera lifecycle is managed by the `cam-*` targets (`cam-up`/`cam-up-wrists`/`cam-down`/`cam-status`); override the rig with `make cam-up CAMERA_CONFIG=/app/config/<rig>.yaml`. The rb_gui wrist point cloud is a separate host process (`camera_server/stereo_worker/wrist_cloud_worker.py`) that reads the D405 depth bundles off `ZMQ 5600` + the shm ring and publishes `stereo.wrist` on `ZMQ 5601`, which `rb_gui/rb_servo_gui/pointcloud_receiver.py` subscribes to. `make cam-up-wrists` starts it and `make cam-down` stops it, so the wrist rig is one command each way (`CLOUD=0` opts out; `make cloud-up`/`cloud-down` drive it alone). No model or GPU — the D405 gives depth directly. It was restored 2026-08-27 after commit `3287da8` deleted the whole `stereo_worker` along with the Fast-FoundationStereo submodule; the head-stereo/box-detect/fusion half of that worker went with FFS and is NOT restored, but the wrist path never depended on it. Flow-matching training runs natively on the GPU server with `python3 -m policy_runner flow-train`.
