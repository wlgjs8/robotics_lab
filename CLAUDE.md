...
- **`policy_runner`** (Python package) owns action sources (`action_sources/`), recording, HDF5 audit, and flow-matching ML training/inference (`flow_*.py`). SpaceMouse Cartesian uses `TcpTwistLocal`, not repeated TCP deltas; `flow-infer` ee_local deltas run as the default `tcp_target_pose` absolute `TcpPoseTarget` setpoints or opt-in `tcp_twist_local` velocity conversion.
