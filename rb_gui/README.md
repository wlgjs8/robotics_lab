...
The scene draws the mechanical Robotous RFT64-6A01 CAD frame. The small
...
yaw. Missing or invalid URDF data disables the overlay instead of substituting
a guessed transform. It is mechanical geometry only — it is not a controller
frame and carries no wrench interpretation. A GUI checkbox controls its
visibility; the overlay is read-only.

Raw external F/T sensor readings (`eft_wrench` / `eft_valid` from the rbpodo
state-frame decode) are shown separately in the operator FT Monitor card, one
axis per row.
