...
56. RBPODO-CIRCLE-ABLATION-OVERRIDES-01 lets the rbpodo circle ablation runner generate per-experiment `resolved_server_config.yaml` files for allowlisted temporary factors such as state publish rate, speed bar, servo rate/t1, command timeout, twist limits, and Cartesian path gains. Source/local configs are not edited, physical-real Cartesian fields remain rejected, and generated configs are audited in each artifact directory.
